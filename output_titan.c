 ...
 * the state written by output_titan_update() keeps being accepted, and
 * is applied as soon as the link is back. */
static volatile int g_link_up = 0;
/* Asked for by output_titan_reset(). Acted on by the send thread,
...
/* How often the rate reported by output_titan_report_rate() -- and so
 ...
 * several milliseconds. output_titan_update() is called from the
 ...
 * continuous loop (see send_thread_main); output_titan_update() just
...
const char *output_titan_protocol_name(int value) {
    return (value >= 0 && value < PROTOCOL_COUNT) ? PROTOCOL_NAME[value] : "";
}

const char *output_titan_protocol_label(int value) {
    return (value >= 0 && value < PROTOCOL_COUNT) ? PROTOCOL_LABEL[value] : "unknown";
}

int output_titan_protocol_from_name(const char *name) {
    if (!name || !*name) {
        return -1;
    }
...
static int send_report(uint8_t type, const uint8_t *data, uint16_t length) {
    ...
    if (ret != 0 && !usb_error_is_disconnect(ret)) {
        fprintf(stderr, "output_titan: failed to send report (type=0x%02x): %s\n", type,
...
static void config_apply_protocol(int wanted, int quiet) {
    ...
    if (config_read(cfg) != 0) {
        if (!quiet) {
            fprintf(stderr, "output_titan: could not read the adapter's configuration\n");
        }
        return;
    }
    ...
    if (written == 0 && config_read(back) == 0) {
        ...
        fprintf(stderr, "output_titan: output protocol now %s%s\n",
                output_titan_protocol_label(back[0]),
                back[0] == wanted ? "" : " (the adapter refused the change)");
    } else {
        fprintf(stderr, "output_titan: failed to set the output protocol\n");
    }
     ...
     * the adapter, which shows as output_titan_console() reading 0, no
    ...
    fprintf(stderr, "output_titan: protocol changed -- if the pad stays dead, unplug the "
 ...
 * deliberately reset. */
/* Defined below, next to output_titan_init() which shares it. */
 ...
 * output_titan_shutdown() joins that thread before sending
...
static int send_thread_main(void *arg) {
    ...
    while (g_thread_running) {
        ...
        if (g_reset_requested) {
            g_reset_requested = 0;
            fprintf(stderr, "output_titan: resetting %s (held closed for %d ms)\n",
        ...
        if (!g_link_up) {
            ...
            if (usb_open_and_claim(1)) {
                fprintf(stderr, "output_titan: %s reconnected, capture mode active\n", g_device_name);
            } else {
        ...
        if (g_home_press_requested) {
            ...
            fprintf(stderr, "output_titan: HOME press requested\n");
        }
        ...
        if (now_ms - stat_since >= SEND_STATS_INTERVAL_MS) {
            ...
            if (app_verbose()) {
                fprintf(stderr, "output_titan: %.0f reports/s, transfer avg %.2f ms / worst %.2f ms\n",
        ...
        if (usb_error_is_disconnect(ret)) {
            ...
            fprintf(stderr, "output_titan: %s disconnected (%s), waiting for it to come back\n",
 ...
 * capture mode. Split out of output_titan_init() so reconnecting after
 ...
 * output_titan_init(), and a failed reconnect attempt must be free to
 * simply be retried a second later. */
static int usb_open_and_claim(int quiet) {
    libusb_device **list;
    ssize_t count = libusb_get_device_list(g_ctx, &list);
    if (count < 0) {
        open_log(quiet, "output_titan: libusb_get_device_list failed\n");
        return 0;
    }
    ...
    if (!target) {
        if (use_config_id) {
            open_log(quiet,
                    "output_titan: no USB device %04lx:%04lx (GAMEPAD_USB_VID/PID in the .env) found, "
                    "gamepad support disabled\n",
                    cfg_vid, cfg_pid);
        } else {
            open_log(quiet, "output_titan: no ConsoleTuner adapter (Titan One...) found, "
                            "gamepad support disabled\n");
        }
    ...
    if (find_hid_endpoints(target, &g_interface_number, &g_out_ep, &g_in_ep) != 0) {
        open_log(quiet, "output_titan: HID interface/endpoint not found on %s\n", target_name);
        libusb_free_device_list(list, 1);
        return 0;
    }

    if (libusb_open(target, &g_handle) != 0) {
        open_log(quiet, "output_titan: could not open %s (udev permissions?)\n", target_name);
        libusb_free_device_list(list, 1);
        return 0;
    }
    libusb_free_device_list(list, 1);
    ...
    if (libusb_claim_interface(g_handle, g_interface_number) != 0) {
        open_log(quiet, "output_titan: could not claim the HID interface of %s\n", target_name);
        libusb_close(g_handle);
        g_handle = NULL;
        return 0;
    }
     ...
     * then ignored button for button. */
    {
        ...
        if (config_read(cfg) == 0) {
            g_output_protocol = cfg[0];
            open_log(quiet, "output_titan: output protocol is %s\n",
                     output_titan_protocol_label(cfg[0]));
        }
    }

    if (send_report(GPPKG_ENTER_CAPTURE, NULL, 0) != 0) {
        open_log(quiet, "output_titan: failed to enter capture mode on %s\n", target_name);
        libusb_release_interface(g_handle, g_interface_number);
        libusb_close(g_handle);
        g_handle = NULL;
        return 0;
...
int output_titan_init(void) {
    ...
    if (libusb_init(&g_ctx) != 0) {
        fprintf(stderr, "output_titan: libusb_init failed\n");
        return 0;
    }
    ...
    if (!g_state_mutex) {
        fprintf(stderr, "output_titan: mutex creation failed\n");
        libusb_release_interface(g_handle, g_interface_number);
        libusb_close(g_handle);
        g_handle = NULL;
        libusb_exit(g_ctx);
    ...
    if (!g_send_thread) {
        fprintf(stderr, "output_titan: SDL_CreateThread failed\n");
        g_thread_running = 0;
    ...
    fprintf(stderr, "output_titan: connected to %s, capture mode active\n", g_device_name);
    g_connected = 1;
    return 1;
}
...
void output_titan_shutdown(void) {
    ...
    fprintf(stderr, "output_titan: left capture mode (%s)\n",
...
/* ------------------------------------------------------------------
 * Generic output-backend adapter.
 * ------------------------------------------------------------------ */

#define OUTPUT_TITAN_MERGED_SOURCE 0x7fff0001u

static void output_titan_backend_update(
    const int8_t state[CONTROLLER_STATE_COUNT])
{
    output_titan_update(OUTPUT_TITAN_MERGED_SOURCE, state);
}

static const GamepadOutputBackend g_output_titan_backend = {
    .name = "titan",
    .init = output_titan_init,
    .update = output_titan_backend_update,
    .reset = output_titan_reset,
    .press_home = output_titan_press_home,
    .link_up = output_titan_link_up,
    .report_rate = output_titan_report_rate,
    .shutdown = output_titan_shutdown,
};

const GamepadOutputBackend *output_titan_backend(void)
{
    return &g_output_titan_backend;
}
